...
class AWSVPCFlowBucket(AWSLogsBucket):
    ...
    def get_flow_logs_ids(self, access_key, secret_key, region, profile_name=None):
        ...
        debug("+++++ Flow logs response: {}".format(ec2_client.describe_flow_logs()), 3)
    ...
    def iter_regions_and_accounts(self, account_id, regions):
        ...
        for base_prefix in self.client.list_objects_v2(Bucket=self.bucket, Delimiter='/')['CommonPrefixes']:
            for vpc_id in self.client.list_objects_v2(Bucket=self.bucket, Prefix=base_prefix['Prefix'], Delimiter='/')['CommonPrefixes']:
                self.prefix = vpc_id['Prefix']
                ...
                debug("+++++ Account IDs: {}".format(account_id), 2)
                for aws_account_id in account_id:
                    # Find which regions exist for this AWS account
                    regions = self.find_regions(aws_account_id)
                    ...
                    for aws_region in regions:
                        ...
                        if not flow_logs_ids:
                            debug("+++ No flow logs", 1)
                        # for each flow log id
                        for flow_log_id in flow_logs_ids:
                            debug("+++++ Flow log id: {}".format(flow_log_id), 4)
                            if self.old_version:
                                self.migrate(aws_account_id=aws_account_id, aws_region=aws_region,
                                            flow_log_id=flow_log_id)
